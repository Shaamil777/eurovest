...
export function DynamicSeoInput(props) {
  const pageName = useFormValue(['pageName']);
  const programSlug = useFormValue(['programSlug']);
  const fieldName = props.schemaType.name; // metaTitle, metaDescription, metaKeywords

  let placeholder = '';

  if (pageName && pageDefaults[pageName]) {
  ...
  } else if (programSlug && programTitles[programSlug]) {
    const title = programTitles[programSlug];
    if (fieldName === 'metaTitle') placeholder = `${title} | Eurovest`;
    if (fieldName === 'metaDescription') placeholder = `Learn more about our ${title} services at Eurovest.`;
    if (fieldName === 'metaKeywords') placeholder = `${title}, eurovest services, investment program, finance`;
  } else {
    placeholder = 'Select a page or program above to see the default value...';
  }

  // Sanity v3 allows us to render the default input and just override elementProps
